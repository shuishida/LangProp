...
def main():
    ...
    while number < args.route_num:
        ...
        start_wp = world_map.get_waypoint(start_wp.transform.location, project_to_road=True,
                                          lane_type=carla.LaneType.Driving)
        end_wp = world_map.get_waypoint(end_wp.transform.location, project_to_road=True,
                                        lane_type=carla.LaneType.Driving)

        if start_wp.transform.location.distance(
                end_wp.transform.location) < min_dist or start_wp.transform.location.distance(
                end_wp.transform.location) > max_dist:
            continue

        wp_list, distance, is_junction = interpolate_trajectory(world_map, [start_wp.transform.location,
        ...
        route = ET.SubElement(route, 'weather', id=weather, cloudiness='%f' % WEATHER_LIST[weather]['cloudiness'],
                              precipitation='%f' % WEATHER_LIST[weather]['precipitation'],
                              precipitation_deposits='%f' % WEATHER_LIST[weather]['precipitation_deposits'],
                              wind_intensity='%f' % WEATHER_LIST[weather]['wind_intensity'],
                              sun_azimuth_angle='%f' % WEATHER_LIST[weather]['sun_azimuth_angle'],
                              sun_altitude_angle='%f' % WEATHER_LIST[weather]['sun_altitude_angle'],
                              fog_density='%f' % WEATHER_LIST[weather]['fog_density'],
                              fog_distance='%f' % WEATHER_LIST[weather]['fog_distance'],
                              fog_falloff='%f' % WEATHER_LIST[weather]['fog_falloff'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_file', type=str, required=False,
                        default="/home/wupenghao/transfuser/leaderboard/data/extra_routes/routes_town10_1000k.xml",
