...
if __name__ == "__main__":
    ...
    run_config = RunConfig(run_name=run_name, n_responses=3, n_top_choices=3, max_keep=3)
    model = train_gym_model(env, Path(__file__).parent, model_name, run_config, epochs=30)
    images, total_reward, _ = eval_gym_policy(model(), env, render=True)
