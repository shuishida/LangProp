...
from baselines.tcp.constants import NORMALIZE_SPEED
from baselines.tcp.learner.model import TCP
from baselines.tcp.learner.data import CARLA_Data
from baselines.tcp.learner.config import GlobalConfig
...
class TCP_planner(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        speed = batch['speed'].to(dtype=torch.float32).view(-1, 1) / NORMALIZE_SPEED
...
if __name__ == "__main__":
    ...
    if args.use_wandb:
        logger = WandbLogger(project="carla", name=args.id)
    else:
        print("Using tensorboard logger instead of wandb.")
