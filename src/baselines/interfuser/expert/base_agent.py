...
class BaseAgent(AgentSuppressPrintMixin, autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        self.track = autonomous_agent.Track.SENSORS
        if path_to_conf_file.endswith("yaml"):
            self.config = yaml.load(open(path_to_conf_file, "r"), Loader=yaml.FullLoader)
        else:
            print(f"{path_to_conf_file} is not a vaild config path. Using default values.")
            self.config = {}
        self.step = -1
        ...
        if SAVE_PATH is not None:
            ...
            if not self.rgb_only:
                for sensor_type in ["seg", "depth", "2d_bbs"]:
