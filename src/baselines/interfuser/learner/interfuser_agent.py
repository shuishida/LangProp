...
from baselines.interfuser.learner.interfuser import create_model
from leaderboard.autoagents import autonomous_agent
from baselines.interfuser.learner.utils import lidar_to_histogram_features, transform_2d_points
from baselines.interfuser.planner import RoutePlanner
from baselines.interfuser.learner.interfuser_controller import InterfuserController
from baselines.interfuser.learner.render import render, render_self_car, render_waypoints
from baselines.interfuser.learner.tracker import Tracker
...
class InterfuserAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        with open(path_to_conf_file, "r") as f:
            py_code = f.read()
        exec(py_code)
        self.config = locals()["GlobalConfig"]()
    ...
    def run_step(self, input_data, timestamp):
        ...
        if SAVE_PATH is not None and self.step % self.config.save_freq == 0:
            self.save(tick_data)

        return control

    def save(self, tick_data):
        frame = self.step // self.config.save_freq
