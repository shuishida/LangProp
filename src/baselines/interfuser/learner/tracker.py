import numpy as np
import math
from baselines.interfuser.learner.render import find_peak_box
    ...
    def _predict(self, updates_ids):
        ...
        for each_id in updates_ids:
            ...
            for i in range(len(to.historical_steps) - 3, -1, -1):
                ...
                speed = 0.5 * self.frequency * np.sqrt(
                    (prev_pos[0] - cur_pos[0]) ** 2 + (prev_pos[1] - cur_pos[1]) ** 2) / (
                                    to.historical_steps[i + 1] - to.historical_steps[i])
