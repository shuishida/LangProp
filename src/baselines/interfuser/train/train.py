...
from baselines.interfuser.learner.interfuser import Interfuser
from baselines.interfuser.train.dataset.carla_dataset import create_carla_dataset
from baselines.interfuser.train.dataset.carla_loader import create_carla_loader
...
from timm.models import (
    ...
    model_parameters, register_model,
)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            output = model(input)
            loss, losses, errors = loss_fns(output, target)
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                ...
                for k, v in losses.items():
                    losses[k] = reduce_tensor(v.data, args.world_size)

            losses_m.update(loss.item(), batch_size)
            for k, v in losses.items():
                metric_summary[k].append(v.item())

            if writer and args.local_rank == 0:
                writer.add_scalar("train/loss", loss.item(), num_updates)
                for k, v in losses.items():
                    writer.add_scalar(
                        f"train/{k}", v.item(), num_updates
                    )
                # Add Image
                writer.add_image(
            ...
            if args.local_rank == 0:
                _logger.info(
                    "Train: {} [{:>4d}/{} ({:>3.0f}%)]  "
                    "Loss(traffic): {loss_traffic:>6.4f}  "
                    "Loss(waypoints): {loss_waypoints:>6.4f}  "
                    "Loss(junction): {loss_junction:>6.4f}  "
                    "Loss(light): {loss_traffic_light_state:>6.4f}  "
                    ...
                    "Data: {data_time.val:.3f} ({data_time.avg:.3f})".format(
                        ...
                        loss=losses_m,
                        loss_traffic=np.mean(metric_summary["loss_traffic"]),
                        loss_waypoints=np.mean(metric_summary["loss_waypoints"]),
                        loss_junction=np.mean(metric_summary["loss_junction"]),
                        loss_traffic_light_state=np.mean(metric_summary["loss_junction"]),
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if args.distributed:
                ...
                for k, v in errors.items():
                    errors[k] = reduce_tensor(v.data, args.world_size)
            ...
            if args.local_rank == 0 and (
                ...
                _logger.info(
                    ...
                    "Loss(junction): {loss_junction:>6.4f}  "
                    "Loss(light): {loss_traffic_light_state:>6.4f}  "
                    "Acc(junction): {junction_errorm:>6.4f}  "
                    "Acc(light): {traffic_light_state_errorm:>6.4f}  ".format(
                        log_name,
                        batch_idx,
                        last_idx,
                        batch_time=batch_time_m,
                        ...
                        loss_junction=np.mean(metric_summary["loss_junction"]),
                        loss_traffic_light_state=np.mean(metric_summary["loss_junction"]),
                        traffic_light_state_errorm=np.mean(metric_summary["traffic_light_state_error"]),
                        junction_errorm=np.mean(metric_summary["junction_error"]),
                        loss=losses_m,
                    )
                )
                if writer:
        ...
        if writer:
            ...
            for k, v in metric_summary.items():
                writer.add_scalar(
                    f"val/{k}", np.mean(v), epoch
                )

    metrics = OrderedDict([("loss", losses_m.avg), ("l1_error", losses_m.avg)])
