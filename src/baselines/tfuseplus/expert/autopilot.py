...
from baselines.tfuseplus.nav_planner import PIDController, RoutePlanner, interpolate_trajectory, extrapolate_waypoint_route
from baselines.tfuseplus.config import GlobalConfig
import baselines.tfuseplus.transfuser_utils as t_u
...
class AutoPilot(autonomous_agent.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file, route_index=None):
        ...
        self.dataaug = int(os.environ.get('DATAAUG', 0)) == 1
    ...
    def run_step(self, input_data, timestamp, sensors=None,
                 plant=False):  # pylint: disable=locally-disabled, unused-argument
        self.step += 1
    ...
    def _get_control(self, input_data, plant=False):
        ...
        data = self.save(target_point, next_target_point, steer, throttle, brake, control_brake, target_speed,
                         tick_data)
    ...
    def _get_brake(self, near_command=None, plant=False):
                ...
                if vehicle.get_location().distance(ego_vehicle_location) < self.config.detection_radius:
                    ...
                    if self.config.model_interactions:
                        ...
                        center_safety_box = traffic_transform.transform(
                            carla.Location(x=traffic_safety_loc[0], y=traffic_safety_loc[1],
                    ...
                    if self.config.model_interactions and self.check_obb_intersection(traffic_safety_box,
                                                                                      ego_bb_global):
                    ...
                    if (statistics.mean(
                            self.vehicle_speed_buffer[vehicle.id]['velocity']) < self.config.stuck_vel_threshold and
                            statistics.mean(self.vehicle_speed_buffer[vehicle.id][
                                                'throttle']) > self.config.stuck_throttle_threshold
                            and statistics.mean(
                                self.vehicle_speed_buffer[vehicle.id]['brake']) < self.config.stuck_brake_threshold):
        ...
        # Stop sign detection
        # -----------------------------------------------------------
        self.stop_sign_hazard = self.ego_agent_affected_by_stop_sign(ego_vehicle_transform, ego_vehicle_location,
                                                                     actors,
    ...
    def check_obb_intersection(self, obb1, obb2):
        ...
        return not (
                ...
                self.get_separating_plane(r_pos, obb2.rotation.get_up_vector(), obb1,
            ...
            obb2) or
                self.get_separating_plane(
                    r_pos, self.cross_product(obb1.rotation.get_forward_vector(), obb2.rotation.get_up_vector()), obb1,
                    obb2) or
                self.get_separating_plane(
                    r_pos, self.cross_product(obb1.rotation.get_right_vector(), obb2.rotation.get_forward_vector()),
                    obb1, obb2)
            ...
            r_pos, self.cross_product(obb1.rotation.get_right_vector(), obb2.rotation.get_up_vector()), obb1, obb2) or
                self.get_separating_plane(
                    r_pos, self.cross_product(obb1.rotation.get_up_vector(), obb2.rotation.get_forward_vector()), obb1,
                    obb2) or
                self.get_separating_plane(
                    r_pos, self.cross_product(obb1.rotation.get_up_vector(), obb2.rotation.get_right_vector()), obb1,
                    obb2) or
