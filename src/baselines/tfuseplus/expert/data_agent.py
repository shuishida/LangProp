...
class DataAgent(AutoPilot):
    ...
    def run_step(self, input_data, timestamp, plant=False):
        ...
        if self.datagen and self.dataaug:
            sensors = self.sensor_interface._sensors_objects
    ...
    def augment_camera(self, sensors):
        ...
        if self.initialized:
            ...
            bb_copy = carla.BoundingBox(self._vehicle.bounding_box.location, self._vehicle.bounding_box.extent)
            transform_copy = carla.Transform(self._vehicle.get_transform().location,
