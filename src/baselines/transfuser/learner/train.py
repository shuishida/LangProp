...
from baselines.transfuser.learner.data import CARLA_Data, lidar_bev_cam_correspondences
...
def main():
    ...
    if ((bool(args.zero_redundancy_optimizer) == True) and (parallel == True)):
        ...
        optimizer = ZeroRedundancyOptimizer(model.parameters(), optimizer_class=optim.AdamW,
                                            lr=args.lr)  # Saves GPU memory during DDP training
    else:
