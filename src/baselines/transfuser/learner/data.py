...
from baselines.transfuser.learner.utils import get_vehicle_to_virtual_lidar_transform, get_vehicle_to_lidar_transform, \
    get_lidar_to_vehicle_transform, get_lidar_to_bevimage_transform
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.seq_len):
            if not self.data_cache is None and str(measurements[i], encoding='utf-8') in self.data_cache:
                measurements_i, images_i, lidars_i, lidars_raw_i, bevs_i, depths_i, semantics_i = self.data_cache[
                ...
                if (backbone == 'geometric_fusion'):
                    lidars_raw_i = np.load(str(lidars[i], encoding='utf-8'), allow_pickle=True)[1][...,
                                   :3]  # lidar: XYZI
                else:
                    lidars_raw_i = None
                lidars_i[:, 1] *= -1
                ...
                if not self.data_cache is None:
                    ...
                    measurements_i, compressed_imgage, lidars_i, lidars_raw_i, compressed_bevs, compressed_depths,
...
def lidar_bev_cam_correspondences(world, lidar_vis=None, image_vis=None, step=None, debug=False):
    ...
    lidar_height = 256
    lidar_meters_x = (
                                 lidar_width / pixels_per_meter) / 2  # Divided by two because the LiDAR is in the center of the image
    ...
    world[:,
    0] *= -1  # flip x axis, so that the positive direction points towards right. new coordinate system: x right, y forward, z up
    ...
    if (debug == True):
        ...
        plt.savefig(r'/home/hiwi/save folder/Visualizations/2/image_with_lidar_{}.png'.format(step),
                    bbox_inches='tight')
        plt.close()
        plt.ioff()
