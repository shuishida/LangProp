...
from baselines.transfuser.learner.utils import *
from baselines.transfuser.learner.models.transfuser import TransfuserBackbone
from baselines.transfuser.learner.models.components.decoders import SegDecoder, DepthDecoder
from baselines.transfuser.learner.models.geometric_fusion import GeometricFusionBackbone
from baselines.transfuser.learner.models.late_fusion import LateFusionBackbone
from baselines.transfuser.learner.models.latentTF import latentTFBackbone
from baselines.transfuser.learner.models.components.point_pillar import PointPillarNet
...
class LidarCenterNetHead(BaseDenseHead, BBoxTestMixin):
    ...
    def loss(self,
        ...
        """
        target_result, avg_factor = self.get_targets(gt_bbox, gt_label, gt_bboxes_ignore, center_heatmap_pred.shape)
    ...
    def get_targets(self, gt_bboxes, gt_labels, gt_ignores, feat_shape):
        ...
        center_heatmap_target = gt_bboxes.new_zeros([bs, self.num_classes, feat_h, feat_w])
        wh_target = gt_bboxes.new_zeros([bs, 2, feat_h, feat_w])
        offset_target = gt_bboxes.new_zeros([bs, 2, feat_h, feat_w])
        yaw_class_target = gt_bboxes.new_zeros([bs, 1, feat_h, feat_w]).long()
        yaw_res_target = gt_bboxes.new_zeros([bs, 1, feat_h, feat_w])
        velocity_target = gt_bboxes.new_zeros([bs, 1, feat_h, feat_w])
        brake_target = gt_bboxes.new_zeros([bs, 1, feat_h, feat_w]).long()
        wh_offset_target_weight = gt_bboxes.new_zeros([bs, 2, feat_h, feat_w])

        for batch_id in range(bs):
            gt_bbox = gt_bboxes[batch_id]
            gt_label = gt_labels[batch_id]
            gt_ignore = gt_ignores[batch_id]
...
class LidarCenterNet(nn.Module):
    ...
    def __init__(self, config, device, backbone, image_architecture='resnet34', lidar_architecture='resnet18',
        ...
        backbone_class = dict(
            transFuser=TransfuserBackbone,
            late_fusion=LateFusionBackbone,
            geometric_fusion=GeometricFusionBackbone,
            latentTF=latentTFBackbone
        )[backbone]

        self._model = backbone_class(config, image_architecture, lidar_architecture,
    ...
    def forward_ego(self, rgb, lidar_bev, target_point, target_point_image, ego_vel, bev_points=None, cam_points=None,
        ...
        results = self.head.get_bboxes(*preds)
        bboxes, _ = results[0]
        ...
        if debug and self.i % 2 == 0 and not (save_path is None):
            pred_bev = self.pred_bev(features)
    ...
    def forward(self, rgb, lidar_bev, ego_waypoint, target_point, target_point_image, ego_vel, bev, label, depth,
        ...
        loss_bbox = self.head.loss(*preds, label, gt_label=gt_labels, gt_bboxes_ignore=gt_bboxes_ignore, img_meta=None)
        ...
        self.i += 1
        if self.config.debug and (self.i % self.config.train_debug_save_freq == 0) and save_path is not None:
            with torch.no_grad():
                results = self.head.get_bboxes(*preds)
                bboxes, _ = results[0]
                bboxes = bboxes[bboxes[:, -1] > self.config.bb_confidence_threshold]
